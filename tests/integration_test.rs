...
fn fetch_pirls_2021_aut_dataset(vars: &[&str], group_vars: &[&str]) -> (Vec<DMatrix<f64>>, Vec<DMatrix<f64>>, DVector<f64>, DMatrix<f64>) {
    ...
    let mut repwgt : DMatrix<f64> = DMatrix::zeros(n_cases, 150);
    let mut data : Vec<DMatrix<f64>> = Vec::new();
    for _ in 0..5 {
    ...
    for (rr, result) in reader.deserialize().enumerate() {
        ...
        for jj in 0..75 {
            repwgt[(rr, jj)] =
                ...
                wgt[rr] * (jk_rep == (jj + 1)) as usize as f64 * jk_rep as f64 * 2.0;
        }
        for jj in 75..150 {
            repwgt[(rr, jj)] =
                wgt[rr] * (jk_zone + 75 != (jj + 1)) as usize as f64 +
                wgt[rr] * (jk_rep + 75 == (jj + 1)) as usize as f64 * (1 - jk_rep) as f64 * 2.0;
        }

        for (vv, var) in vars.iter().enumerate() {
            if record.contains_key(*var) {
                for ii in 0..5 {
                    data[ii][(rr, vv)] = record[*var].parse::<f64>().unwrap();
                }
            } else {
                for ii in 0..5 {
                    let mut varname = var.to_string();
                    varname.push_str(format!("{:02}", ii + 1).as_str());
                    data[ii][(rr, vv)] = record[varname.as_str()].parse::<f64>().unwrap();
                }
            }
        }
        ...
        for (vv, group_var) in group_vars.iter().enumerate() {
            if record.contains_key(*group_var) {
                for ii in 0..5 {
                    groups[ii][(rr, vv)] = record[*group_var].parse::<f64>().unwrap();
                }
            } else {
                for ii in 0..5 {
                    let mut groupvarname = group_var.to_string();
                    groupvarname.push_str(format!("{:02}", ii + 1).as_str());
                    groups[ii][(rr, vv)] = record[groupvarname.as_str()].parse::<f64>().unwrap();
                }
            }
        }
    }
...
#[test]
fn test_analysis_for_pirls_2021_aut() {
    ...
    assert_approx_eq_iter_f64!(result[&vec!["1".to_string()]].final_estimates(), vec![0.24397963, -0.05789637, 654.07200246, 1.0, -0.002617465, 1.0], 8.0);
    assert_approx_eq_iter_f64!(result[&vec!["2".to_string()]].final_estimates(), vec![0.24547254,  0.12003083, 434.22210923, 1.0,  0.012008409, 1.0], 8.0);
    assert_approx_eq_iter_f64!(result[&vec!["3".to_string()]].final_estimates(), vec![0.24889393, -0.40876264, 448.60889779, 1.0, -0.038705965, 1.0], 8.0);
    assert_approx_eq_iter_f64!(result[&vec!["4".to_string()]].final_estimates(), vec![0.24898385, -0.52654363, 426.28373453, 1.0, -0.050899235, 1.0], 8.0);
    assert_approx_eq_iter_f64!(result[&vec!["5".to_string()]].final_estimates(), vec![0.23506757, -0.98851340, 464.08738951, 1.0, -0.093617334, 1.0], 8.0);

    assert_approx_eq_iter_f64!(result[&vec!["1".to_string()]].standard_errors(), vec![0.007082429, 0.976744098, 167.661312585, 0.0, 0.07785359, 0.0], 8.0);
    assert_approx_eq_iter_f64!(result[&vec!["2".to_string()]].standard_errors(), vec![0.003005384, 0.677529944,  24.166076778, 0.0, 0.06555160, 0.0], 8.0);
    assert_approx_eq_iter_f64!(result[&vec!["3".to_string()]].standard_errors(), vec![0.001063050, 0.410383983,  13.879319117, 0.0, 0.03882064, 0.0], 8.0);
    assert_approx_eq_iter_f64!(result[&vec!["4".to_string()]].standard_errors(), vec![0.001165612, 0.486794874,  18.459875205, 0.0, 0.04666934, 0.0], 8.0);
    assert_approx_eq_iter_f64!(result[&vec!["5".to_string()]].standard_errors(), vec![0.010483638, 0.878510479,  59.264497664, 0.0, 0.08172591, 0.0], 8.0);
}